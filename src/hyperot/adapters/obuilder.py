...
class OneBotEventBuilder:
    ...
    def group_sender(
        self, nickname: str, sex: str, age: int, card: str, area: str, level: str, role: str, title: str
    ) -> "OneBotEventBuilder":
        self.data["sender"] = {
    ...
    def private_sender(self, nickname: str, sex: str, age: int) -> "OneBotEventBuilder":
        self.data["sender"] = {"user_id": self.data.get("user_id"), "nickname": nickname, "sex": sex, "age": age}
        return self
...
class OneBotJsonMessageBuilder:
    ...
    def at(self, qq: str):
        self.message.append({"type": "at", "data": {"qq": qq}})
        return self

    def atall(self):
        self.message.append({"type": "at", "data": {"qq": "all"}})
        return self

    def reply(self, message_id: str):
        self.message.append({"type": "reply", "data": {"id": message_id}})
        return self

    def faces(self, face_id: str):
        self.message.append({"type": "face", "data": {"id": face_id}})
        return self

    def record(self, file: str) -> "OneBotJsonMessageBuilder":
        self.message.append({"type": "record", "data": {"file": file, "url": file}})
        return self

    def video(self, file: str) -> "OneBotJsonMessageBuilder":
        self.message.append({"type": "video", "data": {"file": file, "url": file}})
        return self

    def poke(self, poke_type: str, poke_id: str) -> "OneBotJsonMessageBuilder":
        self.message.append({"type": "poke", "data": {"type": poke_type, "id": poke_id}})
        return self

    def contact(self, contact_type: str, contact_id: str) -> "OneBotJsonMessageBuilder":
        self.message.append({"type": "contact", "data": {"type": contact_type, "id": contact_id}})
        return self

    def forward(self, forward_id: str) -> "OneBotJsonMessageBuilder":
        self.message.append({"type": "forward", "data": {"id": forward_id}})
        return self

    def json(self, data: dict | list | str) -> "OneBotJsonMessageBuilder":
        self.message.append(
            {"type": "json", "data": {"data": data if isinstance(data, str) else json.dumps(data, ensure_ascii=False)}}
        )
        return self

    def mface(self, face_id: str, tab_id: str, key: str) -> "OneBotJsonMessageBuilder":
        self.message.append({"type": "mface", "data": {"face_id": face_id, "tab_id": tab_id, "key": key}})
        return self

    def dice(self) -> "OneBotJsonMessageBuilder":
        self.message.append({"type": "dice", "data": {}})
        return self

    def rps(self) -> "OneBotJsonMessageBuilder":
        self.message.append({"type": "rps", "data": {}})
        return self

    def music(
        self, music_type: str, url: str = "", music_id: str = "", title: str = "", audio: str = ""
    ) -> "OneBotJsonMessageBuilder":
        self.message.append(
            {"type": "music", "data": {"type": music_type, "url": url, "audio": audio, "title": title, "id": music_id}}
        )
        return self
